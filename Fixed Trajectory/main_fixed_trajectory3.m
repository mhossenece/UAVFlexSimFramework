...
for traj = 1:maxTrajIter
    ...
    waypoints(2, :) = [lat_drone, lon_drone, 25];
...
best_trajectory = [
    %35.72752574530495	-78.69616739508318	0
    %35.72752574530495	-78.69616739508318	25
    35.72749  -78.6963046 0 %%35.7274934     -78.6963079  0
    35.7274913 -78.6963029 25 %35.7274934     -78.6963079 25
    35.730250742963975	-78.6981208320202	25
    35.72735286298385	-78.70022275170658	25
    35.723510306455935	-78.69314232782715	25
    35.72719710997374	-78.69645491282358	25
    %35.72752574530495	-78.69616739508318	25
    35.7274934     -78.6963079 25
    %35.72752574530495	-78.69616739508318	0
    35.7274934 -78.6963079, 0 %35.7274934     -78.6963079 0
];
...
for i = 1:size(waypoints, 1) - 1
    ...
    if waypoints(i, 3) == 0 && waypoints(i + 1, 3) == 25
        skip = 1
        ascent_distance = abs(25 - 0);
       ...
       % pause_time_ascent = ascent_time / num_steps_ascent;
        ...
        for j = 1:num_steps_ascent
            ...
            for k = 1:1
                ...
                noise_power = -70;
                %distance_to_enb
                %SNR = received_power - noise_power +  randn * 4 %randn %+ snrgp;
                SNR = received_power - noise_power +  randn * 6 %randn %+ snrgp;

                spectral_efficiency = getSpectralEfficiency(SNR);    
                C = spectral_efficiency * 1.4  % Bandwidth in MHz
    ...
    elseif waypoints(i, 3) == 25 && waypoints(i + 1, 3) == 0
        ...
        for j = 1:num_steps_descent
            ...
            for k = 1:1
                ...
                %distance_to_enb
                %SNR = received_power - noise_power +  randn * 4 %randn %+ snrgp;
                %SNR = received_power - noise_power +  rand * 4 %randn %+ snrgp;
                SNR = received_power - noise_power +  randn * 6
        ...
        for j = 1:num_steps            
            ...
            %Did not order sorted_lat_eNBs
            current_lat = lat_points(j);
            current_lon = lon_points(j);
            cureent_alt = alt_points(j);
            ...
            for k = 1: 1
                ...
                timestamp = datestr(now, 'yyyy-mm-dd HH:MM:SS.FFF');
                gps_data = [gps_data; {sprintf('%.15f,%.15f,%.2f', lat_points(j), lon_points(j), alt_points(j))}];
                fprintf(logFileIDs{k}, '%s, %.15f, %.15f, %.15f, %.15f, %.15f, %.15f\n', timestamp, lon_points(j), lat_points(j), alt_points(j), RSRP, SNR, C);
    ...
    if skip == 0
        for i = 1: 10        
            ...
            noise_power = -70;        
            SNR = received_power - noise_power +  randn * 4 %randn %+ snrgp;
            %SNR = received_power - noise_power +  rand * 3 %randn %+ snrgp;
    
            spectral_efficiency = getSpectralEfficiency(SNR);    
            C = spectral_efficiency * 1.4  % Bandwidth in MHz
            timestamp = datestr(now, 'yyyy-mm-dd HH:MM:SS.FFF');
    
            %gps_data = [gps_data; {sprintf('%.15f,%.15f,%.2f', current_lat, current_lon, current_alt)}];
            current_alt = 25
            fprintf(logFileIDs{1}, '%s, %.15f, %.15f, %.15f, %.15f, %.15f, %.15f\n', timestamp, current_lon, current_lat, current_alt, RSRP, SNR, C);
            %only LW1
            pause(1)
        end
    end
...
function se = getSpectralEfficiency(snr)
    if snr >= 21.0 && snr < 22.7
        se = 5.12;
    elseif snr >= 22.7
        se = 5.55;
    elseif snr >= 18.7 && snr < 21.0
        se = 4.52;
    elseif snr >= 16.3 && snr < 18.7
        se = 3.90;
    elseif snr >= 14.1 && snr < 16.3
        se = 3.32;
    elseif snr >= 11.7 && snr < 14.1
        se = 2.73;
    elseif snr >= 10.3 && snr < 11.7
        se = 2.41;
    elseif snr >= 8.1 && snr < 10.3
        se = 1.91;
    elseif snr >= 5.9 && snr < 8.1
        se = 1.48;
    elseif snr >= 4.3 && snr < 5.9
        se = 1.18;
    elseif snr >= 2.4 && snr < 4.3
        se = 0.88;
    elseif snr >= 0.2 && snr < 2.4
        se = 0.60;
    elseif snr >= -2.3 && snr < 0.2
        se = 0.38;
    elseif snr >= -4.7 && snr < -2.3
        se = 0.23;
    elseif snr >= -6.7 && snr < -4.7
        se = 0.15;
    else
        se = 0.0;
    end
