...
def plot_scatter(csv_files, x_column, y_column, y1_color, colors, markers, use_bs_reference=False, fig_num=1):
    ...
    lat_LWs = [35.725, 35.733056]  # Latitude for  LW3, LW4
    lon_LWs = [-78.691667, -78.698333]  # Longitude for LW3, LW4
    ...
    if fig_num == 1:
        ...
        ax1.annotate('Distance to LW3', xy=(37, 7), xytext=(-.2, 7.5), arrowprops=dict(facecolor='darkblue', edgecolor='darkblue', arrowstyle='->', lw=2), fontsize=fontTxt-2, color='darkblue')        
                    #arrowprops=dict(facecolor='black', shrink=0.05), fontsize=fontTxt)
        ax1.annotate('Distance to LW4', xy=(94, 7), xytext=(90, 7.4), arrowprops=dict(facecolor='darkgreen', edgecolor='darkgreen',arrowstyle='->', lw=2), fontsize=fontTxt-2, color='darkgreen')
    ...
    all_labels = [f'TP at LW{idx + 3}' for idx in range(len(csv_files))]# + \
                 #['Distance to LW1)', 'Distance to LW2']
    ax1.legend(handles=all_handles + [line1, line2], labels=all_labels, loc='center right', fontsize=fontTxt-3, frameon=True, ncol=1,
               bbox_to_anchor=(1, .1))

    plt.tight_layout()
    plt.savefig('throughput_distance_vs_time_lw3_lw4.pdf', format='pdf', bbox_inches='tight', dpi=300)
    plt.show()

# Example usage
if __name__ == "__main__":
    # For LW1 and LW2
    csv_files_LW1_LW2 = ["LW3_log.csv", "LW4_log.csv"]
